...
/**
 * NO-LOCK: Multi-threaded version that assigns subsets of the input List<String> for 
 * concurrent processing by separate threads. HashMap is used as an accumulation data 
 * structure for grouping the accumulated temperatures and count of records 
 * by station. It completely ignores any possible data inconsistency due to parallel
 * execution.
 * @author dspatel
 */
public class NoLock
{
	...
	public Map<String, SumCountPair> testAndPrintPerformanceStats(List<String> linesOfFile, boolean fibonacciFlag)
	{
		...
		for(int i=0; i<10; i++)
		{
		    ...
		    Thread thread1 = new Thread(new NoLockRunnable(list1, fibonacciFlag)); 
		    Thread thread2 = new Thread(new NoLockRunnable(list2, fibonacciFlag)); 
		    Thread thread3 = new Thread(new NoLockRunnable(list3, fibonacciFlag)); 
		    Thread thread4 = new Thread(new NoLockRunnable(list4, fibonacciFlag)); 
	...
	public class NoLockRunnable implements Runnable
	{
		...
		public NoLockRunnable(List<String> linesOfFile, boolean fibonacciFlag)
		{
