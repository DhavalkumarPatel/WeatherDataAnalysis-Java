...
/** FINE-LOCK: Multi-threaded version that assigns subsets of the input List<String>
 * for processing by separate threads. ConcurrentHashMap is used as an accumulation data 
* structure for grouping the accumulated temperatures and count of records 
* by station. It locks only the accumulation value objects and not the whole data structure
* and ensures correct multithreaded execution and minimal delays by holding the locks only when
...
{
	private Map<String, SumCountPair> accumulator = new ConcurrentHashMap<>();
	...
	public Map<String, SumCountPair> testAndPrintPerformanceStats(List<String> linesOfFile, boolean fibonacciFlag)
	{
		...
		for(int i=0; i<10; i++)
		{
		    ...
		    Thread thread1 = new Thread(new FineLockRunnable(list1, fibonacciFlag)); 
		    Thread thread2 = new Thread(new FineLockRunnable(list2, fibonacciFlag)); 
		    Thread thread3 = new Thread(new FineLockRunnable(list3, fibonacciFlag)); 
		    Thread thread4 = new Thread(new FineLockRunnable(list4, fibonacciFlag)); 
	...
	public class FineLockRunnable implements Runnable
	{
		...
		public FineLockRunnable(List<String> linesOfFile, boolean fibonacciFlag)
		{
			...
			for(String record : linesOfFile)
			{
				...
				if(tempType.equalsIgnoreCase("TMAX"))
				{
					...
					if(!accumulator.containsKey(stationId))
					{
						...
						// Delay introduced before adding in accumulator 
						if(fibonacciFlag)
						{
							getNthFibonacci(17);
						}
						
						// This is the entry for a station with default value so no problem if other thread overrides it
						accumulator.put(stationId, sumCountPair);
					}	

					// synchronize a specific key in a accumulator and we will always find the key either with real value or with default value
					synchronized (accumulator.get(stationId)) 
					{
						...
						// Update the accumulator with real value
						accumulator.put(stationId, sumCountPair);
					}
				}
			}
